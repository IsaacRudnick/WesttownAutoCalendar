...
async function update_user(email, ical_feed_url) {
  await log_info(`Updating ${email}'s calendar`, 0);
  ...
  await log_info(`Found ${Object.entries(ical_events).length} iCal events`, 1);
  ...
  await log_info(`Found ${gcal_events.length} Google Calendar events`, 1);
  ...
  newline(1);
  await log_info(`Finished updating ${email}'s calendar`, 0);
}
