...
async function update_user(email, ical_feed_url) {
  ...
  if (!ical_events) {
    console.log("🚀 ~ file: calendar_updater.js ~ line 28 ~ update_user ~ gcal_events", gcal_events);
    log_info.log("Error getting iCal events");
    return;
  }
  ...
  if (!gcal_events) {
    log_info("Error getting Google Calendar events");
    return;
  }
